...
export async function GET(request: NextRequest) {
  ...
  let query = supabase
    .from("profiles")
    .select(
      "id, display_name, avatar_url, current_mmr, follower_count, experience_count, follower_verify_status, follower_verified_at, follower_verify_method, profile_interests(category_id, categories(slug, label))",
      { count: "exact" }
    )
