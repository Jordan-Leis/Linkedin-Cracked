...
import { skillsSaveSchema } from "@/lib/validators";
...
export async function POST(request: Request) {
  ...
  const parsed = skillsSaveSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const entries = parsed.data.skills;
  if (entries.length > 0) {
  ...
  const newSkillsCount = entries.length;
